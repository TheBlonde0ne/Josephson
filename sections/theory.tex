...
\begin{equation}
    ...
    \label{eq:jos2}
\end{equation}
\cref{eq:jos1} beschreibt den DC-Josephson-Effekt: Ein supraleitender Gleichstrom bis zu einem Maximalwert $I_c$ kann bei $V=0$ fließen. \cref{eq:jos2} beschreibt den AC-Josephson-Effekt: Liegt eine endliche Spannung $V$ an, ändert sich die Phase linear in der Zeit ($\varphi(t) = \varphi_0 + \omega_J t$), was zu einem Wechselstrom mit der Josephson-Frequenz $f_J = \frac{2e}{h}V$ führt ($\frac{2e}{h}=\SI{483.6}{\giga\hertz\per\milli\volt}$).

\paragraph{DC-Josephsoneffekt}
Für diesen Effekt misst man den Tunnelstrom für $V=0$ bei unterschiedlichen Temperaturen. Nach der 2. Josephson-Gleichung (\cref{eq:jos2}) gilt für $V=0$ direkt $\dot{\varphi} = 0$ und somit $\varphi = \text{const.}$ Da sich die Phasendifferenz stationär auf den eingeprägten Strom einstellt, fließt nach der 1. Josephson-Gleichung (\cref{eq:jos1}) ein zeitlich konstanter Suprastrom $I_s \le I_c$ ohne Spannungsabfall. Übersteigt der Strom die kritische Stromstärke $I_c$, bricht das reine Cooper-Paar-Tunneln zusammen und am Kontakt fällt eine endliche Spannung ab.

\paragraph{AC-Josephsoneffekt}
Liegt am Kontakt eine zeitlich konstante Spannung $V \neq 0$ an, folgt durch Integration der 2. Josephson-Gleichung:
\begin{equation}
    \varphi(t) = \frac{2e}{\hbar} V \cdot t + \varphi_0 = \omega_J t + \varphi_0
\end{equation}
mit der Kreisfrequenz $\omega_J = \frac{2e}{\hbar} V$. Die Phasendifferenz wächst linear in der Zeit, sodass die 1. Josephson-Gleichung einen hochfrequenten Wechselstrom liefert:
\begin{equation}
    I_s(t) = I_c \sin(\omega_J t + \varphi_0)
\end{equation}
...
Für Informationen wurde sich orientiert an: \cite{NicoLang}, \cite{Thesis}