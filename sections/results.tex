...
Mit dem magnetischen Flussquant $\Phi_0 = \frac{h}{2e} \approx 2.0678 \times 10^{-15}\ \text{Vs}$ lässt sich die effektive Barrierefläche \(A_{\text{eff}}\) des Josephson-Kontaktes berechnen \cite{kittel}:

\begin{equation}
    A_{\text{eff}} = \frac{\Phi_0}{\Delta B} = (1.8959 \pm 0.0332)\ \mu\text{m}^2
\end{equation}

Unter Berücksichtigung der magnetischen Eindringtiefe in die beiden supraleitenden Elektroden setzt sich die effektive Fläche aus der Kontaktbreite $w$, der Barrierendicke $D$ und den Londonschen Eindringtiefen $\lambda_{L,1} = \lambda_{L,2} \equiv \lambda_L$ zusammen \cite{Thesis}:
...
\end{equation}
Der theoretische Literaturwert für reines Niob liegt bei $\lambda_{L,\text{Lit}} \approx \SI{39}{\nano\meter}$ \cite{kittel}. Die Abweichung erklärt sich durch Verunreinigungen und reduzierte mittlere freie Weglängen in dünnen Dünnschichtfilmen (Dirty Limit) \cite{Thesis}.
...
Aus der kritischen Stromdichte $j_c$ und der magnetischen Dicke $d_{\text{mag}} = D + 2\lambda_L$ lässt sich die Josephson-Eindringtiefe $\lambda_J$ berechnen gemäß \cite{Thesis}:
