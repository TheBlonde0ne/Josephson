...
Aus den Kennlinien wurde der jeweilige maximale Josephson-Strom bestimmt und gegen die Magnetfeldstärke aufgetragen. Ein repräsentativer Plot ist in \cref{fig:b-dependence} dargestellt. Die Ergebnisse wurden zusätzlich an den theoretischen Verlauf der Fraunhofer-Interferenz (\cref{eq:fraunhofer}) gefittet. Daraus ergaben sich die Fitparameter aus \cref{tab:josephson_fit_ergebnisse}.
...
Der aus dem Fit extrahierte maximale Josephson-Strom bei verschwindendem effektiven Magnetfeld beträgt $I_0 = (59.09 \pm 3.53)\ \mu\text{A}$ (bzw. $0.0591 \pm 0.0035\ \text{mA}$). Dieser Wert ist proportional zur Energielücke \(Delta(T)\) des Supraleiters.
...
Der räumliche Verlauf der supraleitenden Phase $\varphi(x)$ entlang des Kontakts ist in  Näherung linear und wird ausschließlich durch das homogene externe Magnetfeld diktiert ($\frac{\partial \varphi}{\partial x} = \frac{2\pi d_{\text{mag}}}{\Phi_0} B_{\text{ext}}$).
