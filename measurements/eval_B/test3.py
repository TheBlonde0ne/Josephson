...
def check_resistive_state(I, V, r_squared_threshold=0.9994):
    """
...
def evaluate_josephson_file(file_path):
    ...
    # FIX: Define constant with 0 slope so it keeps the [slope, intercept] format
    fit_j2   = np.polyfit(I[idx_j2], V[idx_j2], 1)
    fit_plat   = np.array([0.0, np.mean(V[idx_plat])])
