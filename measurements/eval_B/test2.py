...
def find_intersection_with_error(fit1, cov1, fit2, cov2_b_only=None):
    """
    Finds intersection and propagates standard errors from polyfit.
    cov2_b_only is used specifically for our custom constant j2 fit.
    """
    a1, b1 = fit1[0], fit1[1]
    a2, b2 = fit2[0], fit2[1]
    
    # Extract standard errors
    sig_a1 = np.sqrt(cov1[0, 0])
    sig_b1 = np.sqrt(cov1[1, 1])
    
    if cov2_b_only is not None:
        sig_a2 = 0.0
        sig_b2 = cov2_b_only  # Passed the SEM directly
    else:
        sig_a2 = np.sqrt(cov2[0, 0])
        sig_b2 = np.sqrt(cov2[1, 1])
...
def evaluate_josephson_file(file_path):
    ...
    sig_a1 = np.sqrt(cov_b1[0, 0])  # Error in slope
    sig_b1 = np.sqrt(cov_b1[1, 1])  # Error in intercept
