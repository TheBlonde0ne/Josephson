...
if len(B_mT) > 0:
    ...
    try:
        ...
        # Calculate fit parameter uncertainties (standard errors) from the covariance matrix
        perr = np.sqrt(np.diag(pcov))
        I0_err, B_offset_err, delta_B_err, C_err = perr
        
        # Calculate theoretical area
        PHI_0 = const.h / (2 * const.e)
        Area_eff = PHI_0 / (delta_B_fit * 1e-3)
        
        # Propagate Delta B error to the effective area: 
        # relative error in Area_eff is identical to relative error in delta_B
        Area_eff_err = Area_eff * (delta_B_err / delta_B_fit)
        ...
        # Fit line styled as a solid blue line
        ...
        plt.ylabel('Maximaler Josephson-Strom $I_c$ [mA]', fontsize=12)
        
        plt.grid(True, which='both', linestyle='--', linewidth=0.5, alpha=0.7)
        plt.gca().tick_params(direction='in', top=True, right=True)
        
        # Text box updated to present both parameter values and standard errors
        fit_text = (f"Fit Results:\n"
                    f"$I_0$ = {I0_fit:.4f} $\\pm$ {I0_err:.4f} mA\n"
                    f"$\\Delta B$ = {delta_B_fit:.4f} $\\pm$ {delta_B_err:.4f} mT\n"
                    f"$B_{{offset}}$ = {B_offset_fit:.4f} $\\pm$ {B_offset_err:.4f} mT\n"
                    f"$C$ = {C_fit:.4f} $\\pm$ {C_err:.4f} mA\n\n"
                    f"$A_{{eff}}$ = ({Area_eff*1e12:.2f} $\\pm$ {Area_eff_err*1e12:.2f}) $\\mu$m$^2$")
        ...
        # 7. EXPORT DATA
        # Save measurement points and fitted values
        ...
        # Save parameters along with their standard errors
        param_data = np.column_stack((
            I0_fit, I0_err, 
            B_offset_fit, B_offset_err, 
            delta_B_fit, delta_B_err, 
            C_fit, C_err, 
            Area_eff, Area_eff_err
        ))
        
        param_header = (
            "I0_mA\tI0_err_mA\t"
            "B_offset_mT\tB_offset_err_mT\t"
            "delta_B_mT\tdelta_B_err_mT\t"
            "C_mA\tC_err_mA\t"
            "Area_eff_m2\tArea_eff_err_m2"
        )
        
        np.savetxt("fit_parameters.txt", param_data, header=param_header, fmt="%.6e", comments="")

        print("\nSuccessfully fit Fraunhofer pattern!")
        print(f"I0 = {I0_fit:.4f} ± {I0_err:.4f} mA")
        print(f"delta_B = {delta_B_fit:.4f} ± {delta_B_err:.4f} mT")
        print(f"A_eff = {Area_eff*1e12:.2f} ± {Area_eff_err*1e12:.2f} um^2")
        print("Parameters and errors exported to 'fit_parameters.txt'.")
